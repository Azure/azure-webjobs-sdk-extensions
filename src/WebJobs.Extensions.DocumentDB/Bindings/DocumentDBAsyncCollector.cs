...
namespace Microsoft.Azure.WebJobs.Extensions.DocumentDB
{
    internal class DocumentDBAsyncCollector<T> : IAsyncCollector<T>
    {
        ...
        public async Task AddAsync(T item, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                if (DocumentDBUtility.TryGetDocumentClientException(ex, out DocumentClientException de) &&
                    de.StatusCode == HttpStatusCode.NotFound)
                {
                    ...
                    else
                    {
                        ...
                        string message = $"The collection '{_docDBContext.ResolvedAttribute.CollectionName}' (in database '{_docDBContext.ResolvedAttribute.DatabaseName}') does not exist. To automatically create the collection, set '{nameof(DocumentDBAttribute.CreateIfNotExists)}' to 'true'.";
